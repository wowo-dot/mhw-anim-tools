...
def _choose_integer_candidate(scene_props, controller):
    ...
    for item in scene_props.timl_controller_transforms:
        ...
        if data_type not in INTEGER_TIML_DATA_TYPES:
            continue
        sort_key = (int(item.type_index) * 1000) + int(item.transform_index)
        candidates.append((sort_key, item, binding))
    if not candidates:
        return None, None
...
def _apply_integer_off_grid_edit(scene_props, controller, action):
    analyze_result = bpy.ops.mhw_anim_tools.analyze_timl_controller("EXEC_DEFAULT")
    if "FINISHED" not in analyze_result:
        raise SystemExit("TIML analysis did not finish before the integer quantization smoke step.")

    transform_item, binding = _choose_integer_candidate(scene_props, controller)
    if transform_item is None or binding is None:
        raise SystemExit("Could not find a rebuild-capable integer/bool TIML transform for the quantization smoke test.")
...
def main():
    ...
    if not lmt_path.is_file():
        raise SystemExit("Provide --lmt <path-to-lmt> for the integer TIML quantization smoke test.")
    if not mod3_path.is_file():
        raise SystemExit("Provide --mod3 <path-to-mod3> for the integer TIML quantization smoke test.")
    ...
    try:
        ...
        analyze_result, edit_info = _apply_integer_off_grid_edit(scene_props, controller, timl_action)
        ...
        if "FINISHED" not in inspect_result or "FINISHED" not in import_lmt_result or "FINISHED" not in import_timl_result:
            raise SystemExit("Source import path did not finish successfully before integer TIML quantization export.")
        if "FINISHED" not in analyze_result:
            raise SystemExit("TIML analysis did not finish successfully before integer TIML edit.")
        if "CANCELLED" not in export_result:
            raise SystemExit("Unsafe integer/bool TIML quantization should have been rejected cleanly.")
        messages = "\n".join(item["message"] for item in payload["diagnostics"])
        if "lossy quantization" not in messages.lower():
            raise SystemExit("Expected a clear quantization-risk TIML diagnostic during export.")
    finally:
        addon.unregister()
