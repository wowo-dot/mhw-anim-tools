...
def main():
    ...
    if not lmt_path.is_file():
        raise SystemExit("Provide --lmt <path-to-lmt> for the TIML smoke test.")
    ...
    try:
        ...
        carrier_name = scene_props.last_imported_timl_object_name
        action_name = scene_props.last_imported_timl_action_name
        carrier = bpy.data.objects.get(carrier_name) if carrier_name else None
        action = bpy.data.actions.get(action_name) if action_name else None
        property_names = []
        if carrier is not None and "mhw_anim_tools_timl_property_names" in carrier:
            try:
                property_names = json.loads(carrier["mhw_anim_tools_timl_property_names"])
            except Exception:
                property_names = []
        ...
        payload = {
            ...
            "target_armature": target_armature.name if target_armature else "",
            "carrier_name": carrier.name if carrier else "",
            "action_name": action.name if action else "",
            "carrier_property_names": property_names,
        ...
        if carrier is None:
            raise SystemExit("TIML import did not create a carrier object.")
        if action is None:
            raise SystemExit("TIML import did not create a Blender Action.")
        if not property_names:
            raise SystemExit("TIML carrier has no imported custom properties.")
        if not action.fcurves:
            raise SystemExit("TIML Action contains no fcurves.")
    finally:
        addon.unregister()
