        ...
        for type_index, type_entry in enumerate(entry.types):
            if len(type_entry.transforms) != type_entry.transform_count:
                report.add_error(
                    ...
                    f"Data entry {entry.id} type {type_index} expected {type_entry.transform_count} transforms but parsed {len(type_entry.transforms)}.",
                )
            if type_entry.transform_table_offset >= timl.file_size and type_entry.transform_count:
                report.add_error(
                    "timl.transform_table_offset",
                    f"Data entry {entry.id} type {type_index} transform table points outside the file.",
                )
            for transform_index, transform in enumerate(type_entry.transforms):
                if len(transform.keyframes) != transform.keyframe_count:
                    report.add_error(
                        "timl.keyframe_count",
                        f"Data entry {entry.id} type {type_index} transform {transform_index} expected "
                ...
                if transform.keyframe_table_offset >= timl.file_size and transform.keyframe_count:
                    report.add_error(
                        ...
                        f"Data entry {entry.id} type {type_index} transform {transform_index} keyframe table points outside the file.",
                    )
